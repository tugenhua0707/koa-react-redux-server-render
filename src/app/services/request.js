...
function filterStatus(res) {
  ...
  const error = new Error(typeof global.window === 'undefined' ? res.statusText : config.apiErrorMsg);
...
function filterCode(json) {
  ...
  if (json.code === 10) {
    const error = new Error(typeof global.window === 'undefined' ? json.errorMsg : config.apiUnAuthMsg);
    error.name = config.apiUnAuthMsg;
    throw error;
  } else {
    const error = new Error(json.errorMsg);
    error.name = config.apiErrorMsg;
    throw error;
  }
}
function _fetch (fetchPromise, timeout) {
  ...
  var abortablePromise = Promise.race([
    ...
    const error = new Error(typeof global.window === 'undefined' ? e : config.apiErrorMsg);
    error.name = config.apiErrorMsg;
    throw error;
  });
  setTimeout(function() {
